...
class WifiDirectController {
  ...
  Future initP2p() async {
    ...
    _subscriptions.add(FlutterP2pPlus.wifiEvents.connectionChange!.listen((change) {
      ...
      if (change.wifiP2pInfo.groupOwnerAddress.isNotEmpty) {
        SharedPreferences.getInstance().then((sp) async {
          await sp.setString("ipAddress", change.wifiP2pInfo.groupOwnerAddress);
        });
      }

      debugPrint("[Listen] connectionChange: ${change.wifiP2pInfo.isGroupOwner},"
          " Connected: ${change.networkInfo.isConnected} | _device IP Address: ${change.wifiP2pInfo.groupOwnerAddress}");
    }));
